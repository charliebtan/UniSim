...
def traj_analysis(config):
    ...
    features = tica_features(traj_ref, use_distances=config.use_distances )
    ...
    rmsd_over_time_ref = compute_rmsd_over_time(traj_ref, init_pos, lagtime=config.lag_ref)

    method_list, tics_list, rmsd_over_time_list, valid_list = [], [], [], []
    method_list.append('MD')
    tics_list.append(tics_ref[:, :2])
    
    rmsd_over_time_refs = [rmsd_over_time_ref]
    if config.traj_ref_path_rep:
        for other_ref in config.traj_ref_path_rep:
            traj_ref_rep = load_traj(other_ref, top=top)
            rmsd_over_time_refs.append(compute_rmsd_over_time(traj_ref_rep, init_pos, lagtime=config.lag_ref))
    rmsd_over_time_list.append(rmsd_over_time_refs)
    ...
    for model in config.models:
        ...
        # compute tica
        feat_model = tica_features(traj_model, use_distances=config.use_distances)
        ...
        for other_traj_path in traj_model_path_list:
            ...
            rmsd_over_time_model = compute_rmsd_over_time(other_traj, init_pos, lagtime=config.lag_model)
