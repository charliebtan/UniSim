...
def plot_tic2d_contour(tic0_ref, tic1_ref, tic0_model, tic1_model, save_path, xlabel='TIC 0', ylabel='TIC 1', name="UniSim", kde=None):
...
def plot_free_energy(torsion_ref, torsion_model, save_path, xlabel='TIC 0', name='UniSim'):
    create_save_dir(save_path)
...
def plot_rmsd_over_time(rmsd_list, method_list, save_path):
    ...
    for rmsd_trajs, method in zip(rmsd_list, method_list):
        ...
        # 0.1 ns per frame
        sim_time = np.tile(np.linspace(0, nframe - 1, nframe), n_traj)
    ...
    plt.xlabel("Steps (x100)", color='black', fontsize=50)
