...
class FBMTrainer(Trainer):
    ...
    def share_step(self, batch, batch_idx, val=False):
        ...
        if isinstance(self.model, nn.parallel.DistributedDataParallel):
            loss, (loss_iff, loss_bound) = self.model.module._train(batch, self.config.temperature)
        else:
            loss, (loss_iff, loss_bound) = self.model._train(batch, self.config.temperature)
